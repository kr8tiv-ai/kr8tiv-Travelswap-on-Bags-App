...
describe('ExecutionPolicy', () => {
  ...
  describe('canStartRun()', () => {
    it('allows run when under daily limit', () => {
      const policy = createExecutionPolicy(makeConfig(), conn);
      const result = policy.canStartRun(strategyId);
      expect(result.allowed).toBe(true);
    });

    it('blocks run when kill switch is active', () => {
      const policy = createExecutionPolicy(
        makeConfig({ executionKillSwitch: true }),
        conn,
      );
    ...
    it('blocks run when daily limit is reached', () => {
    ...
    it('allows run when runs are from different strategies', () => {
      const config = makeConfig({ maxDailyRuns: 1 });
      const policy = createExecutionPolicy(config, conn);

      // Insert strategy 2 and a run for it
      conn.run("INSERT INTO strategies (token_mint) VALUES (?)", 'otherMint');
  ...
  describe('canPurchaseGiftCard()', () => {
    it('allows purchase under limits', () => {
      const policy = createExecutionPolicy(makeConfig(), conn);
      const result = policy.canPurchaseGiftCard(strategyId, 50);
      expect(result.allowed).toBe(true);
    });

    it('blocks purchase when kill switch is active', () => {
      const policy = createExecutionPolicy(
        makeConfig({ executionKillSwitch: true }),
        conn,
      );
    ...
    it('blocks purchase exceeding max denomination', () => {
      const config = makeConfig({ giftCardMaxDenomination: 100 });
      const policy = createExecutionPolicy(config, conn);
      const result = policy.canPurchaseGiftCard(strategyId, 150);
      expect(result.allowed).toBe(false);
      expect(result.reason).toContain('exceeds max');
    });

    it('allows purchase at exactly max denomination', () => {
      const config = makeConfig({ giftCardMaxDenomination: 100 });
      const policy = createExecutionPolicy(config, conn);
      const result = policy.canPurchaseGiftCard(strategyId, 100);
      expect(result.allowed).toBe(true);
    });

    it('blocks purchase when daily gift card limit reached', () => {
      const config = makeConfig({ giftCardDailyLimit: 2 });
      const policy = createExecutionPolicy(config, conn);

      // Insert a run to satisfy FK constraint on gift_cards
      const runResult = conn.run(
  ...
  describe('canAllocateBalance()', () => {
    it('allows allocation when under max', () => {
      const policy = createExecutionPolicy(makeConfig(), conn);
      const result = policy.canAllocateBalance('wallet1', 100);
      expect(result.allowed).toBe(true);
    });

    it('blocks allocation when kill switch is active', () => {
      const policy = createExecutionPolicy(
        makeConfig({ executionKillSwitch: true }),
        conn,
      );
    ...
    it('blocks allocation when projected balance exceeds max', () => {
      ...
      const result = policy.canAllocateBalance('wallet1', 200);
      expect(result.allowed).toBe(false);
      expect(result.reason).toContain('exceeds max');
    });

    it('allows allocation when projected balance equals max', () => {
      const config = makeConfig({ balanceMaxUsd: 500 });
      const policy = createExecutionPolicy(config, conn);
    ...
    it('sums balances across all strategies for the same wallet', () => {
      const config = makeConfig({ balanceMaxUsd: 500 });
      const policy = createExecutionPolicy(config, conn);
    ...
    it('handles wallet with no existing balance', () => {
      const config = makeConfig({ balanceMaxUsd: 500 });
      const policy = createExecutionPolicy(config, conn);

      const result = policy.canAllocateBalance('new_wallet', 400);
      expect(result.allowed).toBe(true);
    });
  });
});
